...
server <- function(input, output) {
  ...
  output$distPlot <- renderPlot({
    ...
    p <- ggplot(df, aes(x = x)) +
      ...
      labs(x = expression(theta), y = "Dichte", 
           color = "Verteilung") +
      theme_minimal()
    
    p <- p + annotate("text", x = 0.85, y = max(y_prior, na.rm = TRUE), label = "Prior (Beta)", hjust = 1, color = "blue") +
      annotate("text", x = 0.85, y = max(y_likelihood, na.rm = TRUE), label = "Likelihood (Bernoulli)", hjust = 1, color = "green") +
    ...
    p <- p + annotate("segment", x = hdi_prior[1], xend = hdi_prior[2], y = -0.1, yend = -0.1, colour = "blue", size = 1) +
      annotate("text", x = mean(hdi_prior), y = -0.2, label = "95% HDI (Prior)", colour = "blue") +
      annotate("segment", x = hdi_posterior[1], xend = hdi_posterior[2], y = -0.3, yend = -0.3, colour = "red", size = 1) +
      annotate("text", x = mean(hdi_posterior), y = -0.4, label = "95% HDI (Posterior)", colour = "red") +
      geom_vline(aes(xintercept = mode), linetype = "dashed", color = "black") +
      annotate("text", x = mode, y = -0.2, label = "Modus", hjust = ifelse(mode < 0.5, 1, 0))
  ...
  output$data <- renderUI({
    withMathJax(paste0("$$ \\text{Daten: z} = ", input$z, ", N = ", input$N, " $$"))
  })
}
